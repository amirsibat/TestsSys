 ...
 */
@WebServlet("/CheckExtraTime")
public class CheckExtraTime extends HttpServlet {
    private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public CheckExtraTime() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            int extraMinutes = 0;
            ;
            if (!record.mExtraData.isNull("addExtraMinutes")) {
                extraMinutes = record.mExtraData.getInt("addExtraMinutes");
            }
            if(record.mExtraData.getInt("status") == Record.RecordExamStatus.PENDING_CHECK.ordinal()){
            	extraMinutes = -1;
            }
            response.setStatus(200);
            out.println("{\"success\":" + extraMinutes + "}");
        } catch (Exception e) {
            L.err(e);
            response.setStatus(500);
